...
public class OrthographicCamera extends Camera{
	private float size=1,left,right,bottom,top,near=0.1f,far=100f;

	@Override
	public void update(){
		float aspectRatio=(float)screen.getWidth()/(float)screen.getHeight();
		left=-aspectRatio*size/2;
		right=aspectRatio*size/2;
		bottom=-size/2;
		top=size/2;

		Matrix.orthoM(projectionMatrix.getValues(),0,left,right,bottom,top,near,far);
